...
describe('ProofPlane', () => {
  ...
  describe('logTrustDelta', () => {
    it('should log a trust delta event', async () => {
      ...
      const prevProfile = createProfile({ agentId, adjustedScore: 50, band: TrustBand.T2_CONSTRAINED });
      const newProfile = createProfile({ agentId, adjustedScore: 70, band: TrustBand.T3_TRUSTED });
      ...
      expect(payload.previousScore).toBe(50);
      expect(payload.newScore).toBe(70);
      expect(payload.previousBand).toBe('T2_CONSTRAINED');
      expect(payload.newBand).toBe('T3_TRUSTED');
    });
  });
...
function createDecision(overrides: Partial<Decision> = {}): Decision {
  return {
    ...
    correlationId: uuidv4(),
    permitted: true,
    trustBand: TrustBand.T2_CONSTRAINED,
    trustScore: 55,
